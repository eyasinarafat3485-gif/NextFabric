...
export default function ProductDetails({ params }) {
  ...
  const handleBuyProduct = async () => {
    ...
    // ১. প্রথমে Better Auth থেকে সেশন ডাটা চেক করা হচ্ছে (গুগল সাইন-ইন এর জন্য)
    const sessionData = authClient.useSession?.()?.data || authClient.getSession?.()?.data;
    ...
    try {
      const res = await fetch(`${SERVER_URL}/api/user-collection`, {
        ...
        body: JSON.stringify({
          userEmail, // ডাইনামিক ইউজারের ইমেইল পাঠানো হচ্ছে
          itemId: product.id,
          title: product.title,
          shortDescription: product.shortDescription,
          price: totalPrice,
