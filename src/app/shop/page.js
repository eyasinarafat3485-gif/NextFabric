...
export default function Shop() {
    ...
    async function fetchLiveItems() {
      try {
        ...
        if (res.ok) {
          ...
          if (data && data.length > 0) {
            const mappedData = data.map(item => {
              // Determine category dynamically from tags, title, or description
              let detectedCategory = 'others';
              if (item.category) {
                detectedCategory = String(item.category).toLowerCase();
              } else {
  ...
  // সার্চ বা ফিল্টার বা সর্ট চেঞ্জ হলে পেজ ১ নম্বরে রিসেট করার জন্য
  useEffect(() => {
    setCurrentPage(1);
  }, [search, category, sort]);

  // Filter and Sort Logic (BUG FIXES APPLIED HERE)
  const filteredProducts = [...products]
    .filter((product) => {
      // সেফটি চেইনিং (Optional Chaining) দিয়ে ক্র্যাশ করা আটকানো হলো
      const productTitle = product.title ? product.title.toLowerCase() : '';
      const productDesc = product.shortDescription ? product.shortDescription.toLowerCase() : '';
      const searchTarget = search.toLowerCase();

      const matchesSearch =
        productTitle.includes(searchTarget) ||
        productDesc.includes(searchTarget);

      const matchesCategory =
        category === 'all' ||
        (product.category && product.category === category.toLowerCase());

      return matchesSearch && matchesCategory;
    })
    .sort((a, b) => {
      // ১. প্রাইস নম্বর ফোর্সিং করা হলো নিশ্চিত সর্টিংয়ের জন্য
      ...
      // ২. টাইটেল সেফ কম্পারিজন
      const titleA = a.title || '';
      const titleB = b.title || '';
      if (sort === 'name-az') return titleA.localeCompare(titleB);

      // ৩. নিউয়েস্ট আইটেম (সাম্প্রতিক প্রোডাক্ট সবার আগে দেখানোর জন্য আইডি চেক)
      if (sort === 'newest') {
